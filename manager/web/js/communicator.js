	...
	function CommunicatorLoad()
	{
		...
		__comm_reply_mail = $('comm_my_mail').value;
	...
	function CommunicatorSendMessage(sClearRoomCommand)
	{
		...
		var oMessageEntry = $("MessageEntry");
		...
		var nIM = ($("comm_type_im").checked) ? 1 : 0;
		var nSMS = ($("comm_type_sms").checked) ? 2 : 0;
		var nEmail = ($("comm_type_mail").checked) ? 4 : 0;
		...
		if((sMessage != "" && sMessage != null && typeof sMessage != 'undefined') || (sClearRoomCommand != ""))
		{
			...
			try
			{
				...
				__comm_ajax_req.open("POST", "logic/render.communicatorMsgs.php", true, null, null);
		...
		try
		{
			...
			if(__comm_ajax_req && __comm_ajax_req.readyState == 4)
			{
				...
				if(__httpStatus >= 200 && __httpStatus < 300)
				{
					...
					var oMessageDisplay = $('MessageDisplay');
					...
					if((__comm_new_msg_srch_results > -1 && (__comm_win_blurred || __comm_main_div.style.display == 'none')) || __comm_still_unfocused)
					{
						if(__comm_msg_log != '')
						{
							...
							__comm_win_title_interval_id = window.setTimeout(function () { window.document.title = 'NOVA PORUKA'; var __comm_li = $('comm_li'); __comm_li.style.backgroundImage = 'url(gfx/komunikatorBlink.gif)'; }, 900 );
						}
					}
				}	// "OK" end
				else {
	...
	function CommunicatorGetCanClear()
	{
		...
		{
			$("ClearLog").setAttribute("disabled", "disabled");
			$("comm_type_sms").setAttribute("disabled", "disabled");
			$("comm_type_mail").setAttribute("disabled", "disabled");
		}
		else
		{
			$("ClearLog").removeAttribute("disabled");
			$("comm_type_sms").removeAttribute("disabled");
			$("comm_type_mail").removeAttribute("disabled");
		}
	}
