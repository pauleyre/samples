...
if(isset($_GET['category'])){
	$question_list = $q->get_category_questions($_GET['category'], $offset, $_GET['pp'], 1);
				$total = $dbc->_db->query('SELECT COUNT(id) FROM orbx_mod_ic_question WHERE (category='.$dbc->_db->quote($_GET['category']).') AND (state=1)');
				$total = $dbc->_db->fetch_array($total);
				$total = $total[0];
				$pagination->total = $total;
				$pagination->split_pages();
				$nav .='<p class="pagination">'. $pagination->construct_page_nav(ORBX_SITE_URL.'/?'.$orbicon_x->ptr.'=mod.infocentar&category='.$_GET['category'] . $ascSortLink).'</p>';

} else {
	// * retreive any number of questions sorted by date desceding, default 10
	$question_list = $q->get_latest_questions($offset, $_GET['pp'], 1);
