	...
	function get_news($category, $rows = null, $columns = null)
	{
		...
		while($i < $loop) {
			...
			while($a) {
				...
				$news .= '<div id="nws_' . $category . '_' . $n . '" class="news_box_preview" style="width:' . $css_width . '%;">
                    ' . $img . '
					<h3 class="orbx_news_title">' . $title_a . '</h3>
					<div class="news_date">' . date($_SESSION['site_settings']['date_format'], $a['date']).'</div>
                    <div class="orbx_news_lead">' . $intro.'</div>
					<div class="news_tools">' . $more_a . ' ' . $print_a . '</div>
                </div>';
	...
	function print_news_archive_category($category, $rows = null, $columns = null)
	{
		...
		while($a) {
			...
			$news_archive .= '<div class="orbx_news_archive_item">
				<p class="orbx_news_archive_item_date">' . date($_SESSION['site_settings']['date_format'], $a['date']).'</p>
				' . $img . '
