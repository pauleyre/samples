...
*-----------------------------------------------------------------------*/
	...
	// poll object
	require DOC_ROOT.'/orbicon/modules/polls/class.polls.php';
	$poll = new Poll;

	// poll
	if(isset($_REQUEST['castvote'])) {
		$poll->display_control_links = true;
		echo $poll->cast_poll_vote();
	}
	if(isset($_REQUEST['castvote_survey'])) {
		$poll->display_control_links = true;
		echo $poll->cast_survey_vote();
	}
	else if(isset($_REQUEST['viewresults'])) {
		$poll->display_control_links = (bool) $_REQUEST['show_controls'];
		echo $poll->get_poll_results($_REQUEST['poll']);
	}
	else if(isset($_REQUEST['viewpolls'])) {
		echo $poll->get_poll_vote_options($_REQUEST['poll']);
	}
