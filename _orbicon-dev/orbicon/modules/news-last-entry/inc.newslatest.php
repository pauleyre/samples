...
/**
	...
	function get_last_news_entry()
	{
		...
		if(get_extension($a['image']) == 'swf') {
			...
			$img = (empty($a['image'])) ? null : (empty($url)) ? '<img src="'.ORBX_SITE_URL.'/site/venus/'.$a['image'].'" title="'.$a['title'].'" alt="'.$a['image'].'" class="img" />' : '<p><a class="news_image" href="'.$url.'"><img src="'.ORBX_SITE_URL.'/site/venus/'.$a['image'].'" title="'.$a['title'].'" alt="'.$a['image'].'" class="img" /></a></p>';
		}
		/**
		...
		$more_a = (empty($url)) ? '' : '<a href="'.$url.'">Saznajte više</a>';
		...
		$news = '<h3 class="latest_news_title">'.$title_a.'</h3>
		<p class="date">'.$a['date_text'].'</p>
		<div id="newsLead">' . $img .$intro . '</div>
		<p class="more">'.$more_a.'</p>';
