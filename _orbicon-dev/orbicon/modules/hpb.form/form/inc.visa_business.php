<?php
	...
	{
		return save_visa_business();
	}

	function xml_pdf($id)
	{
		xml_visa_business($id);
	}

	function xml_pdf2($id)
	{
		xml_visa_business2($id);
	}

	function save_visa_business()
	{
		if(isset($_POST['submit'])) {
			$form_id = sql_insert('	INSERT INTO '.TABLE_P_PR1_VB.'
								  (
									naziv, naziv_kartici,
									mb, datum_reg,
								...
								$_POST['limit_racuna'], $_POST['hpb_racun_broj'],
			...
			$form_id2 = sql_insert('	INSERT INTO '.TABLE_P_PR2_VB.'
								  (
									ime_kor, prezime_kor,
									jmbg_kor, ulica_kor,
									mjesto_kor, zip_kor,
									naziv_radnog_mjesta_kor, ime_prezime_kartici_kor,
									trazeni_limit_potrosnje_kor, ime_podnositelj_kor,
								...
								$_POST['trazeni_limit_potrosnje_kor'], $_POST['ime_podnositelj_kor'],
			...
			new_hpbform(TABLE_P_PR1_VB, $form_id);
			new_hpbform(TABLE_P_PR2_VB, $form_id2);

			return $form_id;
		}
	}

	function get_visa_business($id)
	{
		$q = '	SELECT 	*
				FROM 	' . TABLE_P_PR1_VB .'
				WHERE 	(id = %s)
	...
	function get_visa_business2($id)
	{
		$q = '	SELECT 	*
				FROM 	' . TABLE_P_PR2_VB .'
				WHERE 	(id = %s)
	...
	function xml_visa_business($id)
	{
		$posl_kred = get_visa_business($id);
...
<xdp:xdp xmlns:xdp='http://ns.adobe.com/xdp/'>
	<xfa:datasets xmlns:xfa='http://www.xfa.org/schema/xfa-data/1.0/'>
		<xfa:data>
			...
			<form1>
				...
				<limit_racuna>{$posl_kred['limit_racuna']}</limit_racuna>
	...
	<pdf
		href='".ORBX_SITE_URL."/orbicon/modules/hpb.form/pdf/posl/pristupnica_visa_business_kartica.pdf'
		xmlns='http://ns.adobe.com/xdp/pdf/' />
</xdp:xdp>
";
	}

	function xml_visa_business2($id)
	{
		$posl_kred = get_visa_business2($id);
		$posl_kred = array_map('form_replace_zero', $posl_kred);
...
<xdp:xdp xmlns:xdp='http://ns.adobe.com/xdp/'>
	<xfa:datasets xmlns:xfa='http://www.xfa.org/schema/xfa-data/1.0/'>
		<xfa:data>
			...
			<form1>
				...
				<limit_potrosnje>{$posl_kred['naziv_radnog_mjesta_kor']}</limit_potrosnje>
	...
	<pdf
		href='".ORBX_SITE_URL."/orbicon/modules/hpb.form/pdf/posl/pristupnica_visa_business_kartica_2.pdf'
		xmlns='http://ns.adobe.com/xdp/pdf/' />
</xdp:xdp>
";
	}
