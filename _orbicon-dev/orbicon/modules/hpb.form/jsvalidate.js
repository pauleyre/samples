...
function FIC_checkForm(e) {
	...
	if (!all_valid) {
		if (errs.length > 0){
			tt_txt('<span class="red">Nakon što ispravite sljedeće probleme, nastavite sa predajom obrasca:<br /><br /><ul><li>'+errs.join("</li><li>")+'</li></ul></span>');
			//alert("We have found the following error(s):\n\n  * "+errs.join("\n  * ")+"\n\nPlease check the fields and try again");
		} else {
			tt_txt('Neka obvezna polja nisu točno ispunjena. Pregledajte polja sa crvenim okvirom.');
...
function FIC_checkField(c,e) {
    ...
    else if (c.indexOf(' validate-string ') != -1 && !t.match(/^[a-zA-Z ]+$/) && valid) {
		//string bad
		valid = false;
		if(t.length == 0) {
			valid = true;
