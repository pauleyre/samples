...
class Poll
{
	...
	function get_poll_vote_options($poll_permalink, $poll_title = '', $force_disabled = false)
	{
		...
		if(!isset($_COOKIE[$this->get_id_from_permalink($poll_permalink)])) {
			...
			$results_link = ($this->_is_locked($poll_permalink)) ? '' : '<li class="txt"><a id="poll_view_results" href="javascript:;" onclick="javascript:__poll_view_results(\''.$poll_permalink.'\', 1);">'._L('results').'</a></li>';

			if($orbx_mod->validate_module('past-polls')) {
				$past_polls_link = '<li class="txt"><a id="poll_past_polls" href="'.ORBX_SITE_URL.'/?'.$orbicon_x->ptr.'=mod.past-polls'.'">'._L('past_polls').'</a></li>';
				$results_link .= $past_polls_link;
			}

			$ft .= '<div class="poll_ft"><ul>
			<li><a class="poll_cast_vote" href="javascript:;" onclick="javascript:__poll_cast_vote(\''.$poll_permalink.'\');">'._L('vote').'</a></li>
			' . $results_link . '
			</ul></div>';
		}

		return '<p class="poll_title">'.$poll_title.'</p><form action="" id="poll_form_'.$poll_permalink.'" name="poll_form_'.$poll_permalink.'"><div class="poll_bd">'.$poll.'</div>'.$ft.'</form>';
	}
	...
	function get_poll_results($poll_permalink, $poll_title = '')
	{
		...
		$results = '<div class="poll_results">';
		...
		foreach($option as $value) {
			...
			//$voted_in_percent = ($value['votes'] / $votes) * 100;
			//$voted_in_percent_s = str_replace('.', '<span class="h">.', $voted_in_percent);
			$voted_value = $voted_in_percent . ' %';

			$total_values += $voted_in_percent;
			...
			$results .= '<p>'.$value['title'].' <span class="line"><img src="./site/gfx/images/poll_bar.gif" style="width: ' . $width . '%;" alt="' . $voted_value . '" title="' . $voted_value . '"/> <strong>' . $voted_value . '</strong></span></p>';
			$last = $voted_in_percent;
		}

		if(intval($total_values) < 100) {
			$results = str_replace("$last %", ($last + 1) . ' %', $results);
		}
		...
		//$results .= '<p class="answered_by">' . sprintf(_L('poll_answered_by'), $votes) . '</p>';
		$results .= '</div>';

		if($this->display_control_links) {
			$vote_link = (isset($_COOKIE[$this->get_id_from_permalink($poll_permalink)])) ? '' : '<li><a id="poll_view_vote_options" class="poll_vote" href="javascript:;" onclick="javascript:__poll_view_vote(\''.$poll_permalink.'\');">' . _L('return') . '</a></li>';

			if($orbx_mod->validate_module('past-polls')) {
				$past_polls_link = '<li class="txt"><a id="poll_past_polls" href="'.ORBX_SITE_URL.'/?'.$orbicon_x->ptr.'=mod.past-polls'.'">'._L('past_polls').'</a></li>';
				$vote_link .= $past_polls_link;
			}

			$results .= '<div class="poll_ft"><ul>' . $vote_link . '</ul></div>';
		}
	...
	function get_past_polls()
	{
		...
		while($a) {
			$polls .= '<li><a onclick="javascript:__poll_view_results(\''.$a['permalink'].'\', 0);" href="javascript:;">'.$a['title'].'</a></li>';
			$a = $dbc->_db->fetch_assoc($r);
		}
	...
	function get_survey($permalink, $poll_title = '', $force_disabled = false)
	{
		...
		if(!isset($_COOKIE[$this->get_id_from_permalink($permalink)])) {
			...
			$results_link = ($this->_is_locked($permalink) == 1) ? '' : ' <a id="poll_view_results" href="javascript:;" onclick="javascript:__poll_view_results(\''.$permalink.'\', 1);">'._L('results').'</a>';

			$survey .= '<a id="poll_cast_vote" href="javascript:;" onclick="javascript:__survey_cast_vote(\''.$permalink.'\', \''._L('survey_answer_all').'\');">'._L('vote').'</a>&nbsp;' . $results_link;
	...
	function get_survey_results($poll_permalink, $poll_title = '')
	{
		...
		if($this->display_control_links === true) {
			$vote_link = (isset($_COOKIE[$this->get_id_from_permalink($poll_permalink)])) ? '' : '<a id="poll_view_vote_options" href="javascript:;" onclick="javascript:__poll_view_vote(\'' . $poll_permalink . '\');">&laquo; '._L('vote').'</a> ';
			$results .= '<div class="poll_ft">' . $vote_link . '';
		}
