<?php
	...
	$exch_rates_supported = array(
		'AUD' => array('name' => _L('australia'), 'flag' => 'au.gif', 'css' => 'au'),
		'CAD' => array('name' => _L('canada'), 'flag' => 'ca.gif', 'css' => 'ca'),
		'CHF' => array('name' => _L('switzerland'),'flag' => 'ch.gif', 'css' => 'ch'),
		'CZK' => array('name' => _L('czech_republic'), 'flag' => 'cz.gif', 'css' => 'cz'),
		'DKK' => array('name' => _L('denmark'), 'flag' => 'dk.gif', 'css' => 'dk'),
		'EUR' => array('name' => _L('eu'), 'flag' => 'europeanunion.gif', 'css' => 'eu'),
		'GBP' => array('name' => _L('britain'),'flag' => 'gb.gif', 'css' => 'gb'),
		'HUF' => array('name' => _L('hungary'), 'flag' => 'hu.gif', 'css' => 'hu'),
		'JPY' => array('name' => _L('japan'), 'flag' => 'jp.gif', 'css' => 'jp'),
		'NOK' => array('name' => _L('norway'), 'flag' => 'no.gif', 'css' => 'no'),
		'PLN' => array('name' => _L('poland'), 'flag' => 'pl.gif', 'css' => 'pl'),
		'SEK' => array('name' => _L('sweden'), 'flag' => 'se.gif', 'css' => 'se'),
		'SKK' => array('name' => _L('slovaquia'), 'flag' => 'sk.gif', 'css' => 'sk'),
		'USD' => array('name' => _L('usa'),	'flag' => 'us.gif', 'css' => 'us'),
	...
	function save_exch_rates()
	{
		if(
			...
			copy($_FILES['csv']['tmp_name'], DOC_ROOT . '/site/mercury/tecajna.csv');
	...
	function get_first_exch_date($restrict = true)
	{
		...
		if($restrict) {
			$q = '	SELECT 		valid_date
					FROM		'.TABLE_MOD_EXCH_RATES.'
					WHERE		(valid_date <= UNIX_TIMESTAMP()) AND
								(valid_date > 0)
...
//var_dump($a);
	...
	function print_exchange_list($date, $ret = false)
	{
		...
		while($a) {
			...
			$css = $exch_rates_supported[$a['code']]['css'];
	...
	function print_last_exch_menus()
	{
		...
		return print_exch_list_menus($date, true, 'middle_rate', $_GET[$orbicon_x->ptr]);
	}

	function print_exch_list_menus($date, $ret = false, $type = 'middle_rate', $position = '')
	{
		global $orbicon_x;

		$position = (stripos($position, 'hr=') === false) ? '' : $position;
		$options = '';
		...
		$menu = '
		       ...
		       <div id="tecajniCalc">
                <h4>Tečajni kalkulator</h4>
                  <form action="" id="tecajniCalcEntry">
                    <fieldset>
                      <p>
                        <select id="exch_type" name="exch_type" onchange="javascript:update_calc($(\'current_valid_date\').value);">
                         '.$options.'
                        </select>
                        <label for="exch_type">Tečajna lista</label>
                      </p>
                      <p>
                        <input id="valin" name="valin" onchange="javascript:$(\'rezultat\').value = \'\'; this.value = formatNumber_exch(this.value);" type="text" />
                        <label for="valin">Početna vrijednost</label>
                      </p>
                      <p>
                        ...
                        <label for="CurFrom">Valuta / Krajnja valuta</label>
                      </p>
                      <p>

                        <input type="text" class="input_text doubleBorder" value="0,00" name="rezultat" id="rezultat" />
                        <label for="rezultat">Rezultat</label>
                      </p>
                       <p id="error"></p>
                      <input type="button" class="button" value="Izračunaj" onclick="calc();" />
                    </fieldset>
                  </form>
                  <p class="info">Tečajevi iz ove liste primjenjuju se od: '.date($_SESSION['site_settings']['date_format'], $date).'<br />Informacija je isključivo informativna i ničim ne obvezuje Banku.</p>
...
/*		$types = array(
		...
		if($_COOKIE['user_currencies']) {
			$currencies = explode(',', $_COOKIE['user_currencies']);
		}
		else {
			$currencies = explode(',', $cfg['display']['currencies']);
		}
		$currencies = array_remove_empty($currencies);
		...
		foreach($currencies as $currency) {
			...
			//$name = name2css($exch_rates_supported[$currency]['name']);
			$css = $exch_rates_supported[$currency]['css'];
			...
			$table .= '<tr>
	<td title="'.$currency.'" class="'.$css.'">'.$currency.'</td>
	<td>'.$a['buying_2'].'</td>
	<td>'.$a['middle_rate'].'</td>
	<td>'.$a['selling_1'].'</td>
</tr>';
...
'</tbody></table>';
	...
	function name2css($name)
	{
		$css = '';

		switch($name) {
			case _L('australia'): $css = 'australia'; break;
			case _L('canada'): $css = 'canada'; break;
			case _L('czech_republic'): $css = 'czech_republic'; break;
			case _L('denmark'): $css = 'denmark'; break;
			case _L('hungary'): $css = 'hungary'; break;
			case _L('japan'): $css = 'japan'; break;
			case _L('norway'): $css = 'norway'; break;
			case _L('slovaquia'): $css = 'slovaquia'; break;
			case _L('sweden'): $css = 'sweden'; break;
			case _L('switzerland'): $css = 'switzerland'; break;
			case _L('britain'): $css = 'britain'; break;
			case _L('usa'): $css = 'usa'; break;
			case _L('eu'): $css = 'eu'; break;
			case _L('poland'): $css = 'poland'; break;
			case _L('slovenia'): $css = 'slovenia'; break;
			default: $css = ''; break;
		}
	...
	function create_currency_dialog()
	{
		global $orbx_mod, $exch_rates_supported;
		$cfg = $orbx_mod->load_info('exch-rates-summary');
		...
		foreach($exch_rates_supported as $code => $unit) {

			if(($code == 'SIT') || ($code == 'SKK')) {
				continue;
			}
			...
			//if(($code != 'SIT') || ($code != 'SKK')) {
				$country = $unit['name'];
				$css = $unit['css'];
				$checked = (in_array($code, $currencies)) ? ' checked="checked"' : '';

				$dialog .='<li class="'.$css.'"><label for="unit_'.$code.'" class="input_check"><input type="checkbox" name="unit_'.$code.'" id="unit_'.$code.'" class="input_check" value="1" '.$checked.' /> '.$country.'</label></li>';
			//}
		}
