<?php
	...
	if(!get_is_member()) {
		return 'Morate biti prijavljeni da bi pristupili Online Plaćanju';
	}
	...
	if(isset($_GET['finish'])) {
		...
		$pr->change_credits($pr_id, $credits + ($_REQUEST['amount'] / 100));

		redirect(ORBX_SITE_URL . '/?' . $orbicon_x->ptr . '=mod.peoplering&sp=credits');
	}
	...
	(int) $price = str_replace('.', '', $_GET['price']);
	$order_id = 1;

	$mch_code = 'demoshop';
	$signature = md5($mch_code . $order_id . $price . 'EUR' . 'completion=0,retry_count=5' . '');
	$url = ORBX_SITE_URL . '/?' . $orbicon_x->ptr . '=mod.opg';
	$site_url = ORBX_SITE_URL;

	$payment_options = print_select_menu(array(6100 => '50 HRK + PDV', 7930 => '65 HRK + PDV'), $price, true);
...
<input type="hidden" name="disp_currencies" value="HRK,EUR+" />
<input type="hidden" name="payment_policy" value="completion=0,retry_count=5" />
<input type="hidden" name="lang" value="en" />
<input type="hidden" name="signature" value="{$signature}" />
<table>
  <tr>
    <td><label for="amount">Iznos uplate <span class="red">*</span></label></td>
    <td><select id="amount" name="amount" onchange="javascript:redirect('{$url}&amp;price=' + this[this.selectedIndex].value)">{$payment_options}</select></td>
  </tr>
  <tr>
    <td><label for="customer_name">Ime <span class="red">*</span></label></td>
    <td><input type="text" id="customer_name" name="customer_name" value="{$user['first_name']}" /></td>
  </tr>
  <tr>
    <td><label for="customer_surname">Prezime <span class="red">*</span></label></td>
    <td><input type="text" id="customer_surname" name="customer_surname" value="{$user['last_name']}" /></td>
  </tr>
  <tr>
    <td><label for="customer_address">Adresa</label></td>
    <td><input type="text" id="customer_address" name="customer_address" value="{$user['address']}" /></td>
  </tr>
  <tr>
    <td><label for="customer_zip">Poštanski broj</label></td>
    <td><input type="text" id="customer_zip" name="customer_zip" value="{$user['zip']}" /></td>
  </tr>
  <tr>
    <td><label for="customer_city">Grad</label></td>
    <td><input type="text" id="customer_city" name="customer_city" value="{$user['city']}" /></td>
  </tr>
  <tr>
  	<td><label for="customer_country">Zemlja</label></td>
...
<option label="United States Minor Outlying Islands" value="um">United States Minor Outlying Islands</option>
  ...
  <tr>
    <td><label for="customer_telephone">Telefon</label></td>
  ...
  <tr>
    <td colspan="2" align="center"><input type="submit" name="Submit" value="Dalje &raquo;" /></td>
  </tr>
</table>
<p>&nbsp;</p>
<p><img src="{$site_url}/orbicon/modules/opg/gfx/opg.gif" alt="Ouroboros" title="Ouroboros" /></p>
<p><img src="{$site_url}/orbicon/modules/opg/gfx/cards.gif" alt="Credit Cards" title="Credit Cards" /></p>
</form>
TXT;
