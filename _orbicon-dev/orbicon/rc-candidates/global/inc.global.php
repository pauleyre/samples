 ...
 * @copyright Copyright (c) 2007, Pavle Gardijan, Josipa Prikrila 6, 10000 Zagreb (ZG), CROATIA
 * @package Library
 * @subpackage Global
 * @version 1.00
 ...
 */
	 ...
	 * @return string
	 */
	function permalink($str, $urlencode = true)
	{
		// quick exit
		if($str == '') {
			return '';
		...
		if(function_exists('mb_strtolower')) {
			$output = trim(mb_strtolower($str, 'UTF-8'));
		}
		// for singlebyte
		else {
...
cruiser*/
	...
	function str_sanitize($string, $type)
	{
		if(($type == STR_SANITIZE_JAVASCRIPT) || ($type == STR_SANITIZE_XML)) {
			...
			$string = strip_tags($string, '<html><body><head><title><meta><link><a><p><br><ul><ol><li><h1><h2><h3><h4><h5><h6><abbr><acronym><blockquote><q><script><noscript>');
